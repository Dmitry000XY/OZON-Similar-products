...
В этом модуле сохраняются готовые рекомендации после построения.
...
|------------------------------|--------------------------------------------------------------------------|
| [`writers.py`](writers.py)   | сохранение рекомендаций в подробном, обогащённом и compact-формате       |
| [`manifest.py`](manifest.py) | чтение манифеста, поиск путей к артефактам и перенос относительных путей |
...
Именно этот формат дальше удобнее всего использовать в [`serving`](../serving/README.md), потому что для одного товара
уже есть готовый список похожих товаров.
...
Подробнее о контрактах результата и манифеста: [`../../../docs/data_contract.md`](../../../docs/data_contract.md).
...
Для этого в [`manifest.py`](manifest.py) есть helper `rebase_manifest_paths`.
...
Поэтому [`manifest.py`](manifest.py) поддерживает несколько ключей:
...
Сначала [`retrieval`](../retrieval/README.md) и [`business`](../business/README.md) формируют итоговую таблицу
рекомендаций.

Потом `output` сохраняет её в нескольких форматах.

После этого [`serving`](../serving/README.md) может читать `lookup.parquet` и отдавать похожие товары для конкретного
`item_id`.
...
| чтение готовых рекомендаций | [`serving`](../serving/README.md)     |
...
Если меняется `lookup.parquet` или ключи путей в манифесте, нужно проверить [`serving`](../serving/README.md).

Если меняется `detailed.parquet`, нужно проверить [`evaluation`](../evaluation/README.md), потому что метрики читают
рекомендации в подробном формате.
...
|--------------------------------------------------------------------|--------------------------------------------------------|
| [`../pipeline/README.md`](../pipeline/README.md)                   | где output используется в полном запуске               |
...
| [`../serving/README.md`](../serving/README.md)                     | как читать готовый `lookup.parquet`                    |
| [`../evaluation/README.md`](../evaluation/README.md)               | как проверять подробные рекомендации                   |
| [`../../../configs/README.md`](../../../configs/README.md)         | где задаются `outputs.root_dir` и `outputs.latest_dir` |
| [`../../../docs/data_contract.md`](../../../docs/data_contract.md) | контракты итоговых таблиц                              |
| [`../../../scripts/README.md`](../../../scripts/README.md)         | команды запуска и preview                              |
