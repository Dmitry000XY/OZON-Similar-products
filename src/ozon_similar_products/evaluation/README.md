...
|---------------------------|----------------------------------------------------------|
| `split.py`                | разбиение данных на train и validation по датам          |
| `ground_truth.py`         | построение эталонных релевантных пар из будущих действий |
| `metrics.py`              | расчёт offline-метрик                                    |
| `scorecard.py`            | объект с результатом эксперимента                        |
| `tracking.py`             | сохранение JSON и CSV-индекса экспериментов              |
| `validation_cache.py`     | кэширование валидационных пар и ground truth             |
| `validation_semantics.py` | фиксированные правила построения validation ground truth |
...
В `split.py` описано разбиение на обучающую и валидационную части.
...
В `validation_semantics.py` мы фиксируем правила построения validation ground truth.
...
Резервный слой из `business` добавляет рекомендации с `source`, отличным от `behavioral`.

В `metrics.py` отдельно считаются доли таких рекомендаций:
...
`scorecard.py` хранит компактное описание эксперимента.
...
В `tracking.py` находятся функции для сохранения результатов.
...
Чтобы не пересчитывать их при каждом подборе параметров, в `validation_cache.py` есть локальный кэш.
...
| сохранение рекомендаций  | `output`                              |
| подбор параметров        | `cli/run_tune.py` и `configs/tuning/` |
...
|----------------------------------|-----------------------------------------------------------|
| `../retrieval/README.md`         | как строятся рекомендации                                 |
| `../business/README.md`          | как появляются fallback-рекомендации                      |
| `../pipeline/README.md`          | где оценка качества находится в полном запуске            |
| `../../../configs/README.md`     | настройки оценки и подбора параметров                     |
| `../../../scripts/README.md`     | команды `run_full.py`, `run_tune.py`, `compare_tuning.py` |
