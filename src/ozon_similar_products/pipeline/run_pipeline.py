...
def run_scoring_output_from_artifacts(
    ...
    """Run scoring/top-K/fallback/output from prebuilt train artifacts."""
...
def run_pipeline(
    ...
    resolved_run_id = run_id or _run_id(train_until_date=train_until_date, lookback_days=lookback_days)
    outputs_root = _outputs_root(outputs_config)
    resolved_run_dir = Path(output_dir).resolve() if output_dir is not None else outputs_root / "runs" / resolved_run_id
    result = run_scoring_output_from_artifacts(
        config=config,
        ...
        allow_empty_latest_update=allow_empty_latest_update,
    )
